...
namespace Yuicy {
	...
	// used to "uniquely" identify information. In Yuicy, even though we use the term
...
namespace std {
	...
	template <>
	struct hash<Yuicy::UUID>
	{
		std::size_t operator()(const Yuicy::UUID& uuid) const
		{
			// 直接将UUID作为hash值进行返回
			return uuid;
		}
	...
	template <>
	struct hash<Yuicy::UUID32>
	{
		std::size_t operator()(const Yuicy::UUID32& uuid) const
		{
